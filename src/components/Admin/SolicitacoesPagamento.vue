  ...
  <div style="display: flex; width: 100%; flex-direction: row; align-items: flex-start; flex-wrap: wrap">
    <v-flex xs6>
      <v-card style="border-radius: 10px;" color="blue lighten-1" @click="tabs=0" class="ma-2 pa-2" light>
        <h2 class="white--text font-weight-light">Solicitações Pendentes</h2>
      </v-card>
    </v-flex>
    <v-flex xs6>
      <v-card style="border-radius: 10px;" color="green" @click="tabs=1" class="ma-2 pa-2" light>
        <h2 class="white--text font-weight-light">Solicitações Atendidas</h2>
      </v-card>
    </v-flex>
    <v-flex xs12>
      <v-tabs-items
          ...
          <v-flex xs12 md4 v-for="ticket in tickets" v-show="ticket.status === 'pendente' ? true : false" :key="ticket.key">
            ...
            >
              <div style="flex-direction: column; display: flex, align-items: center; justify-content: center; width:100%; flex-wrap: wrap">
                ...
                <h4 class="text-xs-center ma-2 mt-0 mb-4">Cliente: <span style="background-color: #90CAF9; border: 1px solid #344333; border-radius: 5px; padding: 5px;">{{ comprovante.nomeDoUsuario }}</span></h4>
              </div>

              <v-card-actions>
                <v-btn class="" block round color="yellow darken-3">Analisar</v-btn>
              </v-card-actions>
  ...
  methods: {
    pegaSolicitacoes () {
      ...
      query.on('value', async snapshot => {
        ...
        this.solicitacoes = tickets
      })
    },
  },
  created() {
    this.pegaSolicitacoes()
  }
}
</script>
